...
- Name: 05-admin.spec.ts >> Admin Panel >> admin revenue strip shows on dashboard
- Location: tests\05-admin.spec.ts:49:7
...
Locator: locator('text=Pending Approval')
Expected: visible
Error: strict mode violation: locator('text=Pending Approval') resolved to 5 elements:
    1) <a href="/admin/vendors?status=pending" class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors text-white/50 hover:text-white hover:bg-white/8">…</a> aka getByRole('link', { name: 'Pending Approval' })
    2) <p class="text-xs text-gray-400 mt-0.5">0 pending approval</p> aka getByText('0 pending approval')
    3) <p class="text-white/70 text-sm mt-0.5 font-medium">Pending Approval</p> aka getByRole('paragraph').filter({ hasText: /^Pending Approval$/ })
    4) <h2 class="font-semibold text-gray-700 flex items-center gap-2">…</h2> aka getByRole('heading', { name: 'Pending Approvals' })
    5) <p class="text-center text-gray-400 text-sm py-6">No pending approvals 🎉</p> aka getByText('No pending approvals 🎉')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for locator('text=Pending Approval')
